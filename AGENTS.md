...
## [2025-11-27 12:00 UTC] - assistant
Summary of work completed: Added project/node scoped fallbacks for working-memory APIs, updated message queries to join through sessions when no session ID is present, and extended persistence helpers to store node-specific working history outside session context.
Key implementation details or architectural notes: Introduced node-aware keys in `working_memory_parts`, relaxed API validators to accept blank `session_id` when `project_id`/`node_id` are provided, refactored SQL builders in `mysqlQueries` to conditionally join `sessions`, and added Node test coverage verifying both session-scoped and fallback behaviours.
Next steps or handoff notes: Monitor production data for legacy rows missing `node_id`; if encountered, backfill via one-off migration to ensure the new composite key remains unique and message queries stay efficient.
